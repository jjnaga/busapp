...
def run_upsert_procedure(engine: Engine) -> int:
  try:
    ...
    logging.error(f'Calling procedure perform_gtfs_upserts() threw an error: {e.orig}')
    raise
  ...
  with engine.connect() as connection:
    ...
    result = connection.execute(text("""
      ...
      FROM 
        gtfs.last_checked 
    """))
    last_modified_timestamp = result.scalar()  # Fetches the first column of the first row
...
def download_data_if_new(engine: Engine, url: str) -> Optional[bytes]:
  ...
  last_modified_timestamp = datetime.strptime(last_modified_header, "%a, %d %b %Y %H:%M:%S GMT")
  ...
  if last_modified_timestamp <= last_gtfs_metadata:
    logging.info('Downloading data: GTFS file is same or newer as previous ETL. Returning None')
    return None
...
def transform_data(data: BytesIO, file_name: str) -> pd.DataFrame:
  ...
  if file_name == 'stop_times.txt':
    ...
    df['arrival_time'] = pd.to_timedelta(df['arrival_time']).dt.total_seconds().astype(int)
    df['departure_time'] = pd.to_timedelta(df['departure_time']).dt.total_seconds().astype(int)
    # Drop unused columns
...
def main():
  ...
  pg_sqlalchemy_engine = create_engine(f'postgresql://{db_username}:{db_password}@{db_host}:{db_port}/{db_database}')
  ...
  # Download data
  try:
    data_bytes = download_data_if_new(pg_sqlalchemy_engine, url)
  except Exception as e:
    ...
    for csv_file in csv_files:
      ...
      with zip_ref.open(csv_file) as file:
        try:
          ...
          df.to_sql(f'{table_name}_staging', con=pg_sqlalchemy_engine, schema='gtfs', index=False, if_exists='replace')

          logging.info(f'{table_name}: Inserting into staging: OK')
        except Exception as e:
          job_status['message'] = f'Unable to load {csv_file}. Error: {str(e)}'
          return job_status
  ...
  try:
    ...
    job_status['message'] = f'Unable to update last_checked timestamp in gtfs.last_checked: {e}'
    return job_status
